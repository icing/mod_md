...
{
    return store->load_all(mds, store, MD_SG_DOMAINS, "*", MD_SV_MD, p);
}
...
static apr_status_t fs_load_all(apr_array_header_t *mds, md_store_t *store, 
...
static apr_status_t pfs_load(void *baton, apr_pool_t *p, apr_pool_t *ptemp, va_list ap)
{
    ...
    if (APR_SUCCESS == rv) {
        if (pvalue != NULL) {
            ...
            md_log_perror(MD_LOG_MARK, MD_LOG_TRACE3, 0, ptemp, "loading from %s", fpath);
            switch (vtype) {
                case MD_SV_MD:
                           ...
                           const char *dir, const char *name, apr_filetype_e ftype)
{
    ...
    if (APR_SUCCESS == rv) {
        ...
        if (APR_SUCCESS == rv) {
            ...
            APR_ARRAY_PUSH(ctx->mds, md_t *) = md;
            return APR_SUCCESS;
        }
    }
    md_log_perror(MD_LOG_MARK, MD_LOG_WARNING, rv, ptemp, 
                                ...
                                md_store_vtype_t vtype, apr_pool_t *p)
{
    ...
    rv = md_util_files_do(add_md, &ctx, p, s_fs->base, groupname, pattern, filename, NULL);
    if (APR_SUCCESS == rv) {
