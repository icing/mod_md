...
class TestDrive :
    ...
    @pytest.mark.skip(reason="mod_alias redirects fail in TCP port-forwarding scenario")
    def test_500_110(self):
        ...
        expLocation = "http://%s/name.txt" % name
        r = TestEnv.get_meta(name, "/a", useHTTPS=False)
        assert r['http_status'] == 302
        assert r['http_headers']['Location'] == expLocation     # FAIL: mod_alias uses port 5002 (TestEnv.HTTP_PORT)
